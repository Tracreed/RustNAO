...
impl Handler {
    ...
    fn get_source(&self, index: u32) -> Option<constants::Source<'_>> {
        constants::LIST_OF_SOURCES
            .iter()
            .find(|src| src.index == index)
            .cloned()
    }
    ...
    /// let handle = HandlerBuilder::default().api_key("your_api_key").num_results(999).db(999).build();
    /// # tokio_test::block_on(async {
    /// handle.get_sauce("./tests/test.jpg", None, None).await;
    /// # });
    /// ```
    ///
    /// ## Errors
    ...
    /// let handle = HandlerBuilder::default().api_key("your_api_key").num_results(999).db(999).build();
    /// # tokio_test::block_on(async {
    /// handle.get_sauce_as_json("https://i.imgur.com/W42kkKS.jpg", None, None).await;
    /// # });
    /// ```
    ///
    /// ## Errors
    ...
    /// let handle = HandlerBuilder::default().api_key("your_api_key").num_results(999).db(999).build();
    /// # tokio_test::block_on(async {
    /// handle.get_sauce_as_pretty_json("https://i.imgur.com/W42kkKS.jpg", None, None).await;
    /// # });
    /// ```
    ///
    /// ## Errors
